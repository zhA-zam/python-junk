...
def main():
    ...
    print(f'You can go {directions_str}')
    command = get_command().lower()
    while command != 'escape':
        if command == 'go north':
            success = move('north', grid_start, grid)
            if success:
                print('You moved north.')
                command = get_command().lower()   
        if command == 'go south':
            success = move('south', grid_start, grid)
            if success:
                print('You moved south.')
                command = get_command().lower()
        if command == 'go east':
            success = move('east', grid_start, grid)
            if success:
                print('You moved east.')
                command = get_command().lower()
        if command == 'go west':
            success = move('west', grid_start, grid)
            if success:
